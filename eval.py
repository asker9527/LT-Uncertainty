...
save_path = f'./output/eval/{dataset_name}/{ "pretrained" if is_pre else "" }_{model_name}/{method_name}_{optim_name}'
...
# 4. 保存结果
output_csv = os.path.join(results_save_path,'test_results.csv')
df = pd.DataFrame(results)
df.to_csv(output_csv, index=False)
print(f"测试完成，结果保存至 {output_csv}")

output_csv = os.path.join(results_save_path,'train_results.csv')
